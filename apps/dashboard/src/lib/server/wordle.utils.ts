...
export function parse_wordle_message_v2(message: string) {
	...
	const scores: Record<
		string,
		{ failed_mentions: Set<string>; mentions: Set<string>; winners: boolean }
	> = {};
	...
	for (const line of message.split('\n')) {
		...
		const failed_mentions = [...line.matchAll(failed_mention_regex)].map((match) => match[1]);
		const mentions = [...line.matchAll(mention_regex)].map((match) => match[1]);
		if (!winnersAreSet && score !== 'X') {
			winnersAreSet = true;
			scores[score] = {
				winners: true,
				failed_mentions: new Set(failed_mentions),
