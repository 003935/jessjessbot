...
import { Parse_Wordle_Message } from "./wordle.utils";
...
async function sync_wordle_role(winners: Array<GuildMember>, role: Role) {
  const consecutive_winners = new Map<string, GuildMember>(
    winners
      .filter((winner) => role.members.has(winner.id))
  ...
  const to_add = winners.filter(
    (member) => !consecutive_winners.has(member.id),
  );

  if (to_remove.length > 0)
    console.log(
      `Removing role from ${to_remove.map((m) => m.displayName).join(", ")}.`,
    );
  if (to_add.length > 0)
    console.log(
      `Adding role to ${to_add.map((m) => m.displayName).join(", ")}.`,
    );
...
export async function wordle_module(message: Message<boolean>) {
  ...
  if (parse_result.failed_mentions.size > 0) {
    for (const failed_mention of parse_result.failed_mentions) {
      ...
      const user = users
        .filter((member) => member.displayName === failed_mention)
        .first();
      if (user) {
  ...
  if (wordleKingRole === null) {
    console.error(
      `Role with ID ${WORDLE_ROLE_ID} not found in guild ${guild.name}.`,
    );
    return;
  }
  ...
  if (winners_array.length === 0) {
    ...
    console.log(
      `Winners: ${winners_array.map((winner) => winner.displayName).join(", ")}`,
    );
  }
