...
export async function start_background_event_checker(client: Client<true>) {
  ...
  setInterval(async () => {
    ...
    const past_events = (Array.from(event_map.values())).filter((e) => e.scheduledTime.getTime() < time_now)
    console.log(`Found ${past_events.length} past events`);
    if (past_events.length > 0) {
      //delete past events from map
      for (const pe of past_events)
        event_map.delete(pe.id)
      ...
      //send alert messages 
      for (const pe of past_events) {
        console.log(`Sending alert for event ${pe.id}: ${pe.game} at ${pe.scheduledTime}`);
        promises.push(send_alert(client, pe))
      }

      const results = await Promise.allSettled(promises);
      console.log('Promise results:', results.map(r => r.status));
    }else {
