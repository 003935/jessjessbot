...
export class LeagueTable {
  ...
  static async getAccounts(id: string): Promise<User[]> {
    const users = await db
      .select()
      .from(leagueTable)
      .where(eq(leagueTable.discordId, id));
    return users;
  }
  ...
  static async updateAccount(
    id: string,
    league_data: InsertUser["leaguedata"],
  ): Promise<void> {
    await db
      .update(leagueTable)
  ...
  static async leaderboard(limit: number = 7) {
    const users = await db.select().from(leagueTable);
    const ranked = users.filter(
      (u) => u.leaguedata !== null && u.leaguedata.soloq !== undefined,
    );
    const unranked = users.filter(
      (u) => u.leaguedata === null || u.leaguedata.soloq === undefined,
    );
