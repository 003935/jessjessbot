if (process.env.NODE_ENV !== 'production') {
  ...
  async function getSummonerData(gameName, tagLine, region) {
    try {
      ...
      return {
        gameName, tagLine, region: regionUpper, puuid,
  ...
  function getRankValue(tier, rank, lp) {
    const tierValues = {
      ...
      'BRONZE': 1000, 'IRON': 0, 'UNRANKED': -1
    };
    
    const rankValues = { 'I': 400, 'II': 300, 'III': 200, 'IV': 100 };
  ...
  client.on('messageCreate', async (message) => {
    ...
    if (message.content.toLowerCase() === '!wordlestats' || message.content.toLowerCase() === '!kings') {
      const sortedStats = Object.entries(wordleStats).sort((a, b) => b[1] - a[1]).slice(0, 10);
    ...
    if (message.content.toLowerCase().startsWith('!removeplayer ')) {
      ...
      if (index === -1) {
        await message.channel.send(`❌ Player **${gameName}#${tagLine}** not found.`);
        return;
      }
      
      leaguePlayers.splice(index, 1);
      saveLeaguePlayers(leaguePlayers);
      await message.channel.send(`✅ Removed **${gameName}#${tagLine}**.`);
      return;
    }
    
    if (message.content.toLowerCase() === '!leagueranks' || message.content.toLowerCase() === '!lolranks') {
      if (leaguePlayers.length === 0) {
        await message.channel.send('📊 No players being tracked yet! Use `!addplayer name#tag REGION`');
        return;
      }
      ...
      try {
        ...
        await message.channel.send('❌ Error fetching ranks.');
    ...
    for (const winner of winners) {
      if (!wordleStats[winner.id]) wordleStats[winner.id] = 0;
