...
async function main() {
  ...
  while (true) {
    const messages = (await rest.get(
      Routes.channelMessages(values.channelId!),
      {
    ...
    const userMessages = messages.filter(
      (msg) => msg.author.id === WORDLE_BOT_ID,
    );
      ...
      if (parsed.failed_mentions.size > 0) {
        ...
        for (const failed_mention of parsed.failed_mentions) {
          const cached_user_id =
            failed_mentions_to_user_id_map.get(failed_mention);
    ...
    if (last_message === undefined) {
      console.error(
        `messages.last() is undefined, messages.size: ${messages.length}`,
      );
      exit(1);
    }
